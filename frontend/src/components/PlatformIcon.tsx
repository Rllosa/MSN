...
const PLATFORM_CONFIG: Record<string, { bg: string; text: string; label: string; img?: string }> = {
...
export default function PlatformIcon({ platform }: Props) {
  const config = PLATFORM_CONFIG[platform] ?? { bg: "#52525b", text: "#fff", label: platform.charAt(0).toUpperCase() };
